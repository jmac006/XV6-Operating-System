...
struct shm_cnt *counter;
  pid=fork();
    1011:	e8 44 03 00 00       	call   135a <fork>
  sleep(1);
...
struct shm_cnt *counter;
  pid=fork();
    101d:	89 c6                	mov    %eax,%esi
  sleep(1);
    101f:	e8 ce 03 00 00       	call   13f2 <sleep>
...
shm_open(1,(char **)&counter);
    1024:	8d 44 24 2c          	lea    0x2c(%esp),%eax
    1028:	89 44 24 04          	mov    %eax,0x4(%esp)
    102c:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    1033:	e8 ca 03 00 00       	call   1402 <shm_open>
 
  printf(1,"%s returned successfully from shm_open with counter %x\n", pid? "Child": "Parent", counter); 
    1038:	85 f6                	test   %esi,%esi
    103a:	ba 6a 18 00 00       	mov    $0x186a,%edx
    103f:	b8 64 18 00 00       	mov    $0x1864,%eax
    1044:	0f 44 c2             	cmove  %edx,%eax
    1047:	8b 54 24 2c          	mov    0x2c(%esp),%edx
  for(i = 0; i < 10000; i++)
    104b:	31 ff                	xor    %edi,%edi
//we get the virtual address of the page returned into counter
//which we can now use but will be shared between the two processes
...
shm_open(1,(char **)&counter);
  ...
  printf(1,"%s returned successfully from shm_open with counter %x\n", pid? "Child": "Parent", counter); 
    104d:	89 44 24 08          	mov    %eax,0x8(%esp)
    1051:	c7 44 24 04 a4 18 00 	movl   $0x18a4,0x4(%esp)
    1058:	00 
    1059:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    1060:	89 54 24 0c          	mov    %edx,0xc(%esp)
  ...
  for(i = 0; i < 10000; i++)
    ...
    1077:	e8 b4 07 00 00       	call   1830 <uacquire>
     counter->cnt++;
    107c:	8b 44 24 2c          	mov    0x2c(%esp),%eax
    1080:	83 40 04 01          	addl   $0x1,0x4(%eax)
     urelease(&(counter->lock));
    1084:	89 04 24             	mov    %eax,(%esp)
    1087:	e8 c4 07 00 00       	call   1850 <urelease>

//print something because we are curious and to give a chance to switch process
     if(i%1000 == 0)
    108c:	89 f8                	mov    %edi,%eax
    108e:	f7 eb                	imul   %ebx
    1090:	89 f8                	mov    %edi,%eax
    1092:	c1 f8 1f             	sar    $0x1f,%eax
    1095:	c1 fa 06             	sar    $0x6,%edx
    1098:	29 c2                	sub    %eax,%edx
    109a:	69 d2 e8 03 00 00    	imul   $0x3e8,%edx,%edx
    10a0:	39 d7                	cmp    %edx,%edi
    10a2:	75 3e                	jne    10e2 <main+0xe2>
       printf(1,"Counter in %s is %d at address %x\n",pid? "Parent" : "Child", counter->cnt, counter);
    10a4:	8b 54 24 2c          	mov    0x2c(%esp),%edx
    10a8:	b8 64 18 00 00       	mov    $0x1864,%eax
    10ad:	85 f6                	test   %esi,%esi
    10af:	8b 4a 04             	mov    0x4(%edx),%ecx
    10b2:	89 54 24 10          	mov    %edx,0x10(%esp)
    10b6:	c7 44 24 04 dc 18 00 	movl   $0x18dc,0x4(%esp)
    10bd:	00 
    10be:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    10c5:	89 4c 24 1c          	mov    %ecx,0x1c(%esp)
    10c9:	b9 6a 18 00 00       	mov    $0x186a,%ecx
    10ce:	0f 45 c1             	cmovne %ecx,%eax
    10d1:	8b 4c 24 1c          	mov    0x1c(%esp),%ecx
    10d5:	89 44 24 08          	mov    %eax,0x8(%esp)
    10d9:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
    10dd:	e8 de 03 00 00       	call   14c0 <printf>
//which we can now use but will be shared between the two processes
...
shm_open(1,(char **)&counter);
  ...
  printf(1,"%s returned successfully from shm_open with counter %x\n", pid? "Child": "Parent", counter); 
  for(i = 0; i < 10000; i++)
    10e2:	83 c7 01             	add    $0x1,%edi
    10e5:	81 ff 10 27 00 00    	cmp    $0x2710,%edi
    10eb:	75 83                	jne    1070 <main+0x70>
       printf(1,"Counter in %s is %d at address %x\n",pid? "Parent" : "Child", counter->cnt, counter);
}
  ...
  if(pid)
    ...
    10ed:	8b 44 24 2c          	mov    0x2c(%esp),%eax
//print something because we are curious and to give a chance to switch process
     if(i%1000 == 0)
       printf(1,"Counter in %s is %d at address %x\n",pid? "Parent" : "Child", counter->cnt, counter);
}
  ...
  if(pid)
    10f1:	85 f6                	test   %esi,%esi
     {
       printf(1,"Counter in parent is %d\n",counter->cnt);
    10f3:	8b 40 04             	mov    0x4(%eax),%eax
    10f6:	89 44 24 08          	mov    %eax,0x8(%esp)
//print something because we are curious and to give a chance to switch process
     if(i%1000 == 0)
       printf(1,"Counter in %s is %d at address %x\n",pid? "Parent" : "Child", counter->cnt, counter);
}
  ...
  if(pid)
    10fa:	74 2a                	je     1126 <main+0x126>
     {
       printf(1,"Counter in parent is %d\n",counter->cnt);
    10fc:	c7 44 24 04 71 18 00 	movl   $0x1871,0x4(%esp)
    1103:	00 
    1104:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    110b:	e8 b0 03 00 00       	call   14c0 <printf>
    wait();
    1110:	e8 55 02 00 00       	call   136a <wait>
    } else
...
//shm_close: first process will just detach, next one will free up the shm_table entry (but for now not the page)
   shm_close(1);
    1115:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    111c:	e8 e9 02 00 00       	call   140a <shm_close>
   exit();
    1121:	e8 3c 02 00 00       	call   1362 <exit>
  if(pid)
     {
       printf(1,"Counter in parent is %d\n",counter->cnt);
    wait();
    ...
    printf(1,"Counter in child is %d\n\n",counter->cnt);
    1126:	c7 44 24 04 8a 18 00 	movl   $0x188a,0x4(%esp)
    112d:	00 
    112e:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
    1135:	e8 86 03 00 00       	call   14c0 <printf>
    113a:	eb d9                	jmp    1115 <main+0x115>
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    1459:	0f b6 92 07 19 00 00 	movzbl 0x1907(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    15ef:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
    15f1:	b8 00 19 00 00       	mov    $0x1900,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    16b1:	a1 c0 1b 00 00       	mov    0x1bc0,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    16f9:	a3 c0 1b 00 00       	mov    %eax,0x1bc0
}
    16fe:	5b                   	pop    %ebx
    16ff:	5e                   	pop    %esi
    1700:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    1722:	a3 c0 1b 00 00       	mov    %eax,0x1bc0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    174c:	8b 1d c0 1b 00 00    	mov    0x1bc0,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    178b:	3b 15 c0 1b 00 00    	cmp    0x1bc0,%edx
...
morecore(uint nu)
{
  ...
  return freep;
    17c2:	8b 15 c0 1b 00 00    	mov    0x1bc0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    17e4:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
    17e7:	89 15 c0 1b 00 00    	mov    %edx,0x1bc0
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    17fe:	c7 05 c0 1b 00 00 c4 	movl   $0x1bc4,0x1bc0
    1805:	1b 00 00 
    base.s.size = 0;
    1808:	ba c4 1b 00 00       	mov    $0x1bc4,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    180d:	c7 05 c4 1b 00 00 c4 	movl   $0x1bc4,0x1bc4
    1814:	1b 00 00 
    base.s.size = 0;
    1817:	c7 05 c8 1b 00 00 00 	movl   $0x0,0x1bc8
    181e:	00 00 00 
